...
public sealed record ReconciliationLine(DateTimeOffset Date, string Description, decimal Debit, decimal Credit);
...
public static class ReconciliationActExcel
{
    public static byte[] Create(string company, string counterparty, string currency, DateTime from, DateTime to,
        decimal openingDebit, decimal openingCredit, IReadOnlyList<ReconciliationLine> lines)
    {
        ...
        using (var header = ws.Cells[5, 1, 6, 6])
        {
        ...
        ws.Cells[row, 3, row, 6].Style.Font.Bold = true;
        row++;
        ...
        using (var table = ws.Cells[5, 1, row, 6])
        {
        ...
        ws.Cells[7, 3, row, 6].Style.Numberformat.Format = "#,##0.00########";
        ws.Cells[7, 3, row, 6].Style.HorizontalAlignment = ExcelHorizontalAlignment.Right;

        row += 2;
        ws.Cells[row, 2, row, 6].Merge = true;
        ws.Cells[row, 2].Value = closing > 0
            ? $"Задолженность «{counterparty}» перед {company} на {to.AddDays(1):dd.MM.yyyy} составляет {closing:N2} {currency}."
            : closing < 0
                ? $"Задолженность {company} перед «{counterparty}» на {to.AddDays(1):dd.MM.yyyy} составляет {Math.Abs(closing):N2} {currency}."
        ...
        ws.Row(5).Height = 26;
        ws.Row(6).Height = 22;
        ws.Cells[1, 1, row, 6].Style.Font.Name = "Arial";
        ws.Cells[1, 1, row, 6].Style.WrapText = true;
