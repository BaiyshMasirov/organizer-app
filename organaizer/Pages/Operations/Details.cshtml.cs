...
public sealed class DetailsModel(FinanceDbContext db,Dispatcher dispatcher):PageModel
{
 ...
 public async Task<IActionResult> OnGetAsync(Guid id){var operation=await db.Operations.AsNoTracking().Include(x=>x.Counterparty).Include(x=>x.Settlements).ThenInclude(x=>x.Account).ThenInclude(x=>x!.FinancialInstitution).SingleOrDefaultAsync(x=>x.Id==id);if(operation is null)return NotFound();Operation=operation;Company=await db.Companies.AsNoTracking().SingleAsync(x=>x.Id==operation.CompanyId);return Page();}
